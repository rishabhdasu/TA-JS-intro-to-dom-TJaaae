...
heading.style.backColor = `black`;

// Select all the hr elements and store it in a variable named allHrs using querySelectorAll

let allHrs = document.querySelector(`hr`);
...
let allHrsArray = Array.from(NodeList);
...
allHrsArray.forEach ( elem => elem.style.border = "antiquewhite" );

// Change the 'border-radius' of all the hr to "5px" using array.

allHrsArray.forEach ( elem => elem.style.borderRadius = "antiquewhite" );
...
two.append("Append inserts as last child")
...
three.prepend("Prepend inserts as first child");
...
let boxBlue = document.querySelectorAll(".box")
boxBlue.forEach((elm,index) =>{
  if((index + 1) % 2 !== 0){
